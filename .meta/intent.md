...
- Work with any asset backend, not just `@nextlake/assets`
...
- **Structural typing, no import.** The `AssetSource` interface and `createAssetSource` adapter use structural typing (duck typing) — no runtime import of `@nextlake/assets`. Any object matching the `AssetManagerLike` shape works. This avoids a hard dependency and keeps the package tree clean.
...
- **Block definitions included.** `imageBlock` and `videoBlock` are defined here using `@nextlake/schema` so the schema engine is the only dependency. These blocks standardise how images and videos are referenced in content.

## Constraints

- Depends on `@nextlake/schema` (for block definitions and field types)
- No runtime dependency on `@nextlake/assets`
